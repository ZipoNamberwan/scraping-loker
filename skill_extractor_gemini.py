...
class SkillExtractorGemini:
    def __init__(self, api_key=None):
        """
        Initialize the skill extractor with Google Gemini API
        """
        if api_key:
    ...
    def extract_skills_from_text(self, description):
        """
        Extract skills from a job description using Google Gemini
        """
        prompt = f"""
...
"""
    ...
    def save_results(self, skills_df, results_df, summary_df, output_file="skill_extraction_gemini.xlsx"):
        """
...
def main():
    """
    Main function to run skill extraction with Google Gemini
    """
    print("=== Job Skill Extractor with Google Gemini ===")
    print("Using Gemini 1.5 Flash (FREE tier)")
    print("-" * 50)
    
    # Configuration
    input_file = "combined_description.xlsx"
    output_file = "skill_extraction_gemini.xlsx"
    
    # Get API key
    api_key = input("Enter your Google Gemini API key (or press Enter if set in environment): ").strip()
    if not api_key:
        api_key = os.getenv('GEMINI_API_KEY')
        if not api_key:
            print("❌ No API key provided. Please set GEMINI_API_KEY environment variable or enter it when prompted.")
    ...
    try:
        ...
        extractor = SkillExtractorGemini(api_key)
        
        # Ask for processing limits
        print(f"\nProcessing options:")
        print(f"1. Process all descriptions (FREE with Gemini)")
        ...
        print(f"\n🚀 Starting skill extraction with Google Gemini...")
        ...
        print(f"❌ Error: {e}")
        print("\n💡 Troubleshooting:")
        print("   1. Make sure you have the google-generativeai package installed:")
